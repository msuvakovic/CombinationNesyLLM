...
class Pipeline:
    ...
    def sanitize_asp(text: str) -> str:
        if not text:
            return ""
        text = text.replace("```asp", "").replace("```", "").replace("prolog", "")
        lines = []
        for line in text.splitlines():
            ...
            if line.lower().startswith("semantic parse"):
                continue
        # keep only actual rules/facts/comments
    ...
    def generalize_external_traj(self, save=True):
        ...
        rules = self.sanitize_asp(rule_sets)
        rules = self.keep_only_parseable_rules(rules)        
        if save:
